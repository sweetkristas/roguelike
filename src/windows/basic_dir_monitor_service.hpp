...
public: 
    ...
    { 
        if (!boost::filesystem::is_directory(dirname) && !boost::filesystem::is_other(dirname))
        ...
        BOOL res = ReadDirectoryChangesW(ck->handle, ck->buffer, sizeof(ck->buffer), TRUE, 0x1FF, &bytes_transferred, &ck->overlapped, NULL); 
        if (!res) 
        { 
            delete ck; 
            DWORD last_error = GetLastError(); 
